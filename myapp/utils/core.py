...
def fix_task_position(pipeline,tasks,expand_tasks):
    ...
    # if max_deep>1:
    #     for deep in range(2,max_deep+1):
    #         # 每个节点的y与deep有关，x与所有相同deep的节点数量有关
    #         start_y+=150
    #         # 获取相同deep的节点数组
    #         all_root_tasks=[[] for x in range(root_num)]  # 按不同的数，获取当前deep的叶子节点
    #         for task_name in dag_json:
    #             if dag_json[task_name]['deep']==deep:
    #                 all_root_tasks[dag_json[task_name]['index']].append(task_name)
    #         print(all_root_tasks)
    #         start_x = 50
    #         for root_tasks in all_root_tasks:
    #             for task_name in root_tasks:
    #                 task_id = str(tasks[task_name]['id'])
    #                 # 获取当前节点所有父节点的最小x
    #                 upstreams_tasks_x = [read_position(tasks[task_name]['id'])[0] for task_name in dag_json[task_name]['upstream']]
    #                 print(upstreams_tasks_x)
    #
    #                 # upstreams_tasks_x = [int(dag_json[task_name]['position']['x']) for task_name in dag_json[task_name]['upstream']]
    #                 # 节点与上游最左侧父节点平齐
    #                 start_x = max(start_x,min(upstreams_tasks_x))
    #                 set_position(task_id, start_x, start_y)
    #                 start_x+=400
